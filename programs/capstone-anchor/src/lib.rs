use anchor_lang::prelude::*;
use anchor_lang::system_program::{transfer, Transfer};

declare_id!("ieBaBaedCadehYhjhesE8yo4pAfdT82t4Z3Y8ic9b1n");

#[program]
pub mod capstone_anchor {
    use super::*;

    pub fn init_jar(ctx: Context<InitJar>) -> Result<()> {
        let jar = &mut ctx.accounts.jar;
        jar.creator = ctx.accounts.creator.key();
        jar.total_raised = 0;
        jar.donation_count = 0;
        jar.last_donor = Pubkey::default();
        jar.bump = ctx.bumps.jar;
        Ok(())
    }

    pub fn donate(ctx: Context<Donate>, amount: u64) -> Result<()> {
        require!(amount > 0, TipJarError::ZeroAmount);

        let cpi_ctx = CpiContext::new(
            ctx.accounts.system_program.to_account_info(),
            Transfer {
                from: ctx.accounts.donor.to_account_info(),
                to: ctx.accounts.jar.to_account_info(),
            },
        );
        transfer(cpi_ctx, amount)?;

        let jar = &mut ctx.accounts.jar;
        jar.total_raised = jar
            .total_raised
            .checked_add(amount)
            .ok_or(TipJarError::Overflow)?;
        jar.donation_count = jar
            .donation_count
            .checked_add(1)
            .ok_or(TipJarError::Overflow)?;
        jar.last_donor = ctx.accounts.donor.key();
        Ok(())
    }

    pub fn withdraw(ctx: Context<Withdraw>, amount: u64) -> Result<()> {
        require!(amount > 0, TipJarError::ZeroAmount);

        let jar_info = ctx.accounts.jar.to_account_info();
        let creator_info = ctx.accounts.creator.to_account_info();

        // PDAs owned by our program can't move lamports via the System Program.
        // Keep enough lamports for rent-exemption; transfer the rest by direct
        // lamport mutation, which Anchor allows for program-owned accounts.
        let rent_minimum = Rent::get()?.minimum_balance(jar_info.data_len());
        let available = jar_info.lamports().saturating_sub(rent_minimum);
        require!(amount <= available, TipJarError::InsufficientFunds);

        **jar_info.try_borrow_mut_lamports()? -= amount;
        **creator_info.try_borrow_mut_lamports()? += amount;
        Ok(())
    }
}

#[derive(Accounts)]
pub struct InitJar<'info> {
    #[account(
        init,
        payer = creator,
        space = 8 + TipJar::INIT_SPACE,
        seeds = [b"jar", creator.key().as_ref()],
        bump
    )]
    pub jar: Account<'info, TipJar>,
    #[account(mut)]
    pub creator: Signer<'info>,
    pub system_program: Program<'info, System>,
}

#[derive(Accounts)]
pub struct Donate<'info> {
    #[account(
        mut,
        seeds = [b"jar", jar.creator.as_ref()],
        bump = jar.bump
    )]
    pub jar: Account<'info, TipJar>,
    #[account(mut)]
    pub donor: Signer<'info>,
    pub system_program: Program<'info, System>,
}

#[derive(Accounts)]
pub struct Withdraw<'info> {
    #[account(
        mut,
        seeds = [b"jar", creator.key().as_ref()],
        bump = jar.bump,
        has_one = creator
    )]
    pub jar: Account<'info, TipJar>,
    #[account(mut)]
    pub creator: Signer<'info>,
}

#[account]
#[derive(InitSpace)]
pub struct TipJar {
    pub creator: Pubkey,
    pub total_raised: u64,
    pub donation_count: u64,
    pub last_donor: Pubkey,
    pub bump: u8,
}

#[error_code]
pub enum TipJarError {
    #[msg("Donation amount must be greater than zero")]
    ZeroAmount,
    #[msg("Overflow on jar counters")]
    Overflow,
    #[msg("Insufficient withdrawable funds (rent-exempt minimum must remain)")]
    InsufficientFunds,
}
